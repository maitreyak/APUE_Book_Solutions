...
Select and Poll system call need support from the underlying device driver. The calling process is put to sleep and is awakened by the OS scheudler using the underlying ```wait_queue``` OS implemtation, when the file descriptor accociated would not block. However, a real understanding of the poll & select system calls on only be achived by understanding linux device drivers. See http://www.xml.com/ldd/chapter/book/ch05.html#t3 
